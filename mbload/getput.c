...
/*modified by Mike Ferrara mikef@sr.hp.com for HP-UX upto 57600 baud */
...
int seconds;
{
    ...
    if ((select(ttyin+1, &readfd, (fd_set *) 0, (fd_set *) 0, &tmout)) == 0)
	return (TIMEOUT);
...
{
#ifdef USE_BUFFERED
    if (fwrite(&data, 1, 1, ttyfp) != 1)	/* write the byte */
	error("Write error on stream", TRUE);
    fflush(ttyfp);
#else
    if (write(ttyout, &data, 1) != 1)	/* write the byte (assume it goes NOW; no flushing needed) */
	error("Write error on stream", TRUE);
    tcdrain(ttyout);
#endif
    return;
...
{
#ifdef USE_BUFFERED
    if (fwrite(buffer, 1, nbytes, ttyfp) != nbytes)	/* write the buffer */
	error("Write error on stream", TRUE);
    fflush(ttyfp);
#else
    if (write(ttyout, buffer, nbytes) != nbytes)	/* write the buffer (assume no TIOCFLUSH needed) */
	tcdrain(ttyout);
    error("Write error on stream", TRUE);
#endif
    return;
...
int milliseconds;
{
    ...
    (void) select(ttyin+1, &readfd, (fd_set *) 0, (fd_set *) 0, &timeout);
}
...
int ttyspeed;
{
	...
	0, B0, 50, B50, 75, B75, 110, B110,
	    ...
	    38400, B38400, 57600, B57600, -1, -1
    };

    extern onintr();

    /* Validate the input speed is valid.  If not return false */
    for (speedIx = 0; speeds[speedIx].speed != ttyspeed; ++speedIx) {
	if (speeds[speedIx].speed < 0) {
	    return FALSE;
	}
    }
    sleep(2);			/* let the output appear */
    if (ioctl(ttyin, TCGETA, &ttys) < 0)	/* get tty params */
	error("Can't get TTY Parameters", TRUE);

    tty = ttyname(ttyin);	/* identify current tty */

    if (ioctl(ttyin, TCGETA, &ttysnew) < 0)	/* get tty params */
	error("Can't get TTY Parameters", TRUE);
    ttysnew.c_cc[VMIN] = 1;	/* VMIN */
    ttysnew.c_cc[VTIME] = 0;	/* VTIME */
    ttysnew.c_iflag = 0;
    ttysnew.c_oflag = 0;
    ttysnew.c_lflag = 0;
...
#ifdef CRTSCTS
    ttysnew.c_cflag &= ~CRTSCTS;
#endif
    if (speedIx) {
	ttysnew.c_cflag &= ~CBAUD;
	ttysnew.c_cflag |= speeds[speedIx].speed_symb;
    }
    if (ioctl(ttyin, TCSETA, &ttysnew) < 0)	/* set new paramters */
